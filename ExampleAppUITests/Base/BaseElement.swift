...
//        }
    ...
    func clearText() {
        ...
        if(BaseTest().app.keyboards.count == 1) {
            BaseTest().app.keys["delete"].tap()
            } else{
             BaseTest().app.menuItems["Cut"].tap()
        }
    }
    ...
    func verifyPresent(fMsg: String? = "The element did not appear even after 10 seconds") {
        XCTAssertTrue(waitForPredicate(element: self, predicate: NSPredicate(format: "exists == true"), timeout: 10) == .completed , fMsg!)
    }
    
    /// Waits for an element to be enabled.
    ///
    /// - Parameter fMsg: Message to be displayed on failure
    func verifyEnabled(fMsg: String? = "The element is not enabled even after 10 seconds"){
        XCTAssertTrue(waitForPredicate(element: self, predicate: NSPredicate(format: "enabled == true AND hittable == true"), timeout: 10) == .completed,fMsg!)
    }
    
    /// Waits for an element to be disabled.
    ///
    /// - Parameter fMsg: Message to be displayed on failure
    func verifyDisabled(fMsg: String? = "The element is not disabled even after 10 seconds"){
        //        let envProps : [String: AnyObject] = DataUtil().loadEnvProp()!
        XCTAssertTrue(waitForPredicate(element: self, predicate: NSPredicate(format: "enabled == false"), timeout: 10) == .completed,fMsg!)
    }
    
    /// Waits for an element to disappear.
    ///
    /// - Parameter fMsg: Message to be displayed on failure
    func verifyDisappeared(fMsg: String? = "The element did not get disabled even after 10 seconds"){
        XCTAssertTrue(waitForPredicate(element: self, predicate: NSPredicate(format: "exists == false"), timeout: 10) == .completed, fMsg!)
    }
